...
export function DesignLab() {
  ...
  // Per-step gating so users can't advance with incomplete choices.
  const canAdvance = useMemo(() => {
    switch (step) {
      case 0:
        return state.inspiration.trim().length > 0 || state.imageAdded
      case 1:
        return state.sizeId !== ''
      case 2:
        return state.yarnId !== '' && state.mainColor !== ''
      case 3:
  ...
  const goTo = (i: number) => setStep(Math.max(0, Math.min(i, LAB_STEPS.length - 1)))
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="rounded-3xl border border-border bg-card p-6 sm:p-8 lg:p-10">
        {step === 0 && <StepInspiration state={state} update={update} />}
        {step === 1 && <StepSize state={state} update={update} />}
        {step === 2 && <StepYarn state={state} update={update} />}
        {step === 3 && (
          <StepHandles state={state} update={update} yarn={selectedYarn} />
        )}
        {step === 4 && <StepSummary state={state} onEdit={() => goTo(0)} />}
      </div>
      ...
      {!isLast && (
        <div className="mt-6 flex items-center justify-between gap-4">
          ...
          <Button
            ...
            onClick={() => goTo(step + 1)}
          >
            {step === 3 ? 'See Design Summary' : 'Continue'}
